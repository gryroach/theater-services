...
class GenreService:
    ...
    async def get_all_genres(self, page_size: int, page_number: int) -> list[Genre]:
        ...
        try:
            response = await self.elastic.search(
                index=EsIndexes.genres.value, body=body
            )
    ...
    async def get_genre_by_id(self, genre_id: UUID) -> Genre | None:
        try:
            response = await self.elastic.get(
                index=EsIndexes.genres.value, id=str(genre_id)
            )
    ...
    async def get_popular_films(
        ...
        try:
            response = await self.elastic.search(index=EsIndexes.movies.value, body=body)
