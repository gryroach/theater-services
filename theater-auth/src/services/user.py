...
from exceptions.user_exceptions import UserAlreadyExistsError, UserDoesNotExistsError
...
class UserService:
    ...
    async def register_user(
        ...
        existing_user = await self.user_repo.get_by_field(db, "login", user_data.login)
        if existing_user:
    ...
    async def get_user_by_id(self, db: AsyncSession, user_id: UUID) -> UserInDB:
    ...
    async def update_role(
        ...
        if not user:
            raise UserDoesNotExistsError("User does not exists")
        updated_user = await self.user_repo.update(db, db_obj=user, obj_in=new_role)
    ...
    async def update_user_data(
        ...
        if not user:
            raise UserDoesNotExistsError
        updated_user = await self.user_repo.update(db, db_obj=user, obj_in=user_data)
        return UserData.model_validate(updated_user)

    async def register_user_by_oauth(
        self, db: AsyncSession, user_data: UserOauthRegister, provider: str
    ) -> tuple[User, str | None]:
        """
        Создание пользователя через его почту со случайным паролем.
        Если пользователь уже существует, он возвращается без пароля.
        """
        provider_field = self._get_provider_field(provider)

        user_social_network = await self.user_network_repo.get_by_field(
            db, provider_field, user_data.provider_id
        )
        ...
        await self.user_network_repo.create(
            ...
            obj_in=UserSocialNetworkCreate(
                user_id=user.id, **{provider_field: user_data.provider_id}
            ),
        )
        return user, user_password

    async def attach_social_network(
        self, db: AsyncSession, user_id: str, provider: str, provider_id: str
    ) -> None:
        user = await self.user_repo.get_by_field(db, "id", user_id)
        if not user:
            raise UserDoesNotExistsError("User not found")

        provider_field = self._get_provider_field(provider)

        await self.user_network_repo.create_or_update_provider(
            db,
            obj_in=UserSocialNetworkCreate(
                user_id=user.id, **{provider_field: provider_id}
            ),
            provider_field=provider_field,
        )

    @staticmethod
    def _get_provider_field(provider: str) -> str:
        provider_service = PROVIDERS.get(provider)
        if not provider_service:
            raise InvalidProviderError(f"Unsupported provider: {provider}")
        return provider_service[1]
...
async def get_user_service(
    ...
    return UserService(
        user_repo=user_repo,
        user_network_repo=user_network_repo,
